...
class LoginWindow(Tk):
    ...
    def creat_File(self):
        ...
        # 创建需要的各种文件
        tip1 = {'说明': [{'若直接删除分类': '会将类内所有书都删掉'}]}
        tip2 = [{"添加的句读将会在此显示\n": "\n"}]
        try:
            with open(self.user_path, mode="w", encoding='UTF-8') as fd:
                fd.write("")
            with open("D:\\BookAttic\\Bookname.json", 'w', encoding='UTF-8') as f:
                json.dump(tip1, f, ensure_ascii=False)
            with open("D:\\BookAttic\\Judou.json", "w", encoding='UTF-8') as f:
                json.dump(tip2, f, ensure_ascii=False)
        except:
    ...
    def setup_GUI(self):
        ...
        # 插入图片
        self.bkgif = PhotoImage(file="D:\BookAttic\pic.gif")
    ...
    def AddUser(self):
        ...
        if account == '' or pwd == '' or again == '':
            ...
            if pwd == again:
                ...
                with open(self.user_path, mode="a", encoding='UTF-8') as fd:
                    fd.write(done)
                fd.close()
                self.load_file_info()
                self.au.destroy()
    ...
    def load_file_info(self):
        ...
        if not os.path.exists(self.user_path):
            ...
            try:
                with open(file=self.user_path, mode="r", encoding='UTF-8') as fd:
                    # 读取第一行
    ...
    def write_file_info(self):
        ...
        """
        try:
            with open(file=self.user_path, mode="w", encoding='UTF-8') as fd:
                fd.write("")
            with open(file=self.user_path, mode="a", encoding='UTF-8') as fd:
